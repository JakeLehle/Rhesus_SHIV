...
BASE_TAG = {'T cells': 'Tcell', 'B cells': 'Bcell', 'Monocytes': 'Mono'}.get(LINEAGE, 'lineage')
VERSION  = 'v2'              # recovered-pool re-run; keeps v1 tables intact
TAG = BASE_TAG               # CSV/panel names stay tier2_Tcell_*

OUT_DIR   = os.path.join(WORKING_DIR, 'annotation_output', f'tier2_{BASE_TAG}_{VERSION}')
ADATA_OUT = os.path.join(WORKING_DIR, 'shiv_starsolo', 'merged',
                         f'shiv_{BASE_TAG}_subset_reclustered_{VERSION}.h5ad')
...
# %% ============================================================
# CELL 8b - RECOVERED-CELL TRACKING (did TCR-recovered cells make a junk bin?)
# ============================================================
print("\n" + "=" * 80)
print("CELL 8b - RECOVERED-CELL TRACKING")
print("=" * 80)

if 'is_tcr_recovered' in sub.obs.columns:
    rec = sub.obs['is_tcr_recovered'].astype(bool)
    print(f"  recovered T cells in this subset: {int(rec.sum()):,} / {sub.n_obs:,} "
          f"({100*rec.mean():.1f}%)")

    per = sub.obs.groupby('sub_leiden').agg(
        n_cells=('sub_leiden', 'size'),
        n_recovered=('is_tcr_recovered', 'sum'))
    per['pct_recovered'] = (100 * per['n_recovered'] / per['n_cells']).round(1)

    hb_genes = present(sub, ['HBB', 'HBA1', 'HBA2'])
    if hb_genes:
        layer = COUNTS_LAYER if have_counts else None
        hb_pos = pd.Series(gset_sum(sub, hb_genes, layer) >= 1, index=sub.obs_names)
        per['pct_HBpos'] = (100 * hb_pos.groupby(sub.obs['sub_leiden']).mean()).round(1)
    print("  per-subcluster recovered fraction + HB content:")
    print(per.to_string())
    save_csv(per, f'tier2_{TAG}_recovered_by_subcluster.csv')

    if 'tcr_recovery_source' in sub.obs.columns:
        src = sub.obs.loc[rec, ['sub_leiden', 'tcr_recovery_source']]
        src_tab = pd.crosstab(src['sub_leiden'], src['tcr_recovery_source'])
        print("\n  recovered-cell SOURCE lineage by subcluster:")
        print(src_tab.to_string())
        save_csv(src_tab, f'tier2_{TAG}_recovered_source_by_subcluster.csv')
    print("  => a subcluster that is mostly recovered-from-Erythrocyte/Platelet")
    print("     with high HBpos is a contamination bin -> exclude like old c10.")
else:
    print("  is_tcr_recovered not in obs (running on a non-recovered object?)")
...
for ax, key, ttl in zip(axes.ravel(),
    ...
    if key in sub.obs.columns:
        sc.pl.umap(sub, color=key, ax=ax, show=False, title=ttl, frameon=False,
                   size=16, legend_loc='on data' if key in ('sub_leiden',) else 'right margin',
                   legend_fontsize=20)
plt.tight_layout()
...
if DO_PREBBKNN and 'X_umap_pre' in sub.obsm:
    ...
    for ax, key, ttl in zip(axes, ['X_umap_pre', 'X_umap'],
        ...
        sc.pl.embedding(sub, basis='X_umap_tmp', color='_animal', ax=ax,
                        show=False, title=ttl, frameon=False, size=16,
                        legend_fontsize=20)
